...
namespace FUKinectTool
{
    ...
    public class FUKinectHelper
    {
        ...
        public bool TakeColorPicture()
        {
            ...
            var angle = new AxisAngleRotation3D();
    ...
    public class FUPostureDetector
    {
        ...
        public bool IsFloorVisible1(AllFramesReadyEventArgs e)
        {
            using (SkeletonFrame skeletonFrame = e.OpenSkeletonFrame())
            {
                if (skeletonFrame == null || e == null)
        ...
        /// <param name="skeleton">Tracked skeleton.</param>
        /// 
        public double GetDistanceFromFloor1(AllFramesReadyEventArgs e, Joint joint)
        {
            using (SkeletonFrame skeletonData = e.OpenSkeletonFrame())
            {
        ...
        public double sitcon(AllFramesReadyEventArgs e, Joint joint,Joint joint1)
        {
            using (SkeletonFrame skeletonData = e.OpenSkeletonFrame())
            {
                ...
                double x1 = joint.Position.X;
                double y1 = joint.Position.Y;
                double z1 = joint.Position.Z;
                double x2 = joint1.Position.X;
                double y2 = joint1.Position.Y;
                double z2 = joint1.Position.Z;
                double delx = x1 - x2;
                double dely = y1 - y2;
                double delz = z1 - z2;
             delx=   Math.Sqrt(Math.Pow(delx, 2));
             dely = Math.Sqrt(Math.Pow(delx, 2));
             delz = Math.Sqrt(Math.Pow(delx, 2));
            
                distanceFromFloor = floorClipPlaneX * delx + floorClipPlaneY * dely + floorClipPlaneZ * delz + floorClipPlaneW;
                return distanceFromFloor;
            }
        }
